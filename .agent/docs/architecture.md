...
- `package.json` version changes are release metadata only; each push-live version bump must stage the matching promoted `CHANGELOG.md` section before branch promotion so updater surfaces, release automation, and branch builds read the same version narrative. The current promoted release metadata is `0.11.4`, dated 2026-07-31 in the changelog.
...
- `renderer/src/hooks/useImportWorkbenchActions.ts` and `renderer/src/hooks/useImportWorkbenchScan.ts` share the persisted importer ignore-key contract from `renderer/src/types/importer.ts`, so ignored scan identities are loaded from preferences and filtered before later scans create staged queue entries.
- `renderer/src/hooks/useImportWorkbenchScan.ts` and `main/ipc/scanningHandlers.ts` treat saved install directories, executable parent directories, and nested path variants as duplicate installations, keeping launcher subfolders such as Ubisoft's `bin_plus` from being surfaced as new games.
...
- `renderer/src/hooks/useAppShellLibraryFilters.ts` keeps category pinning user-driven instead of auto-pinning every category discovered in the library.
- `main/UserPreferencesService.ts` preserves direct Card / Poster preference writes when normalizing canonical `cardView` sections, so Poster Smart Fill survives reloads.
