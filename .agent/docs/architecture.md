...
- `package.json` version changes are release metadata only; each push-live version bump must stage the matching promoted `CHANGELOG.md` section before branch promotion so updater surfaces, release automation, and branch builds read the same version narrative. The current promoted release metadata is `0.11.3`, dated 2026-07-31 in the changelog.
