...
- `package.json` version changes are release metadata only; the push-live flow promotes the matching `CHANGELOG.md` entry before branch promotion so updater surfaces, release automation, and branch builds read the same version narrative.
...
- `main/main.ts`, `main/ipc/appHandlers.ts`, and `main/ui/tray.ts` share tray preference flags so minimize-to-tray, close-to-tray, tray visibility, and tray restore all read the same saved settings. Tray restore shows, focuses, and reapplies the prior maximized/fullscreen state instead of reopening a restored-size window.
