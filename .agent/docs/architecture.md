...
- `package.json` version changes are release metadata only; the push-live flow promotes the matching `CHANGELOG.md` entry before branch promotion so updater surfaces and release automation read the same version narrative.
...
- `renderer/src/hooks/useControllerNavigation.ts` owns browser Gamepad API polling, repeat throttling, connection/input diagnostics, and standard/non-standard button and D-pad input normalization for the main library shell, routing DualSense-style input through the same active-game selection, context-menu, top-bar, and details-action callbacks used by mouse input.
