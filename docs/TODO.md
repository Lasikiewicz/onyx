...
- [ ] **Split `OnyxSettingsModal.tsx` by tab/workflow** - Break [`OnyxSettingsModal.tsx`](../renderer/src/components/OnyxSettingsModal.tsx) into tab-level components plus shared settings state/persistence helpers so the modal is no longer a single giant UI surface.
  Current progress: all tab bodies now live in dedicated settings components (`SettingsGeneralTab.tsx`, `SettingsAnimationsTab.tsx`, `SettingsIntegrationsTab.tsx`, `SettingsAboutTab.tsx`, `SettingsLibrariesTab.tsx`, `SettingsAdvancedTab.tsx`, `SettingsLinksTab.tsx`, `SettingsScanningTab.tsx`, and `SettingsSuspendTab.tsx`), `useOnyxSettingsModalShellState.ts` now owns tab routing/updater/API-status/suspend-capture behavior, and `useOnyxSettingsLibrarySources.ts` now owns launcher/manual-folder loading and library-source actions; the remaining work is the final save/reset orchestration still embedded in `OnyxSettingsModal.tsx`.
