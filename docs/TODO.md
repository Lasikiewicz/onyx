...
- [ ] **Split `OnyxSettingsModal.tsx` by tab/workflow** - Break [`OnyxSettingsModal.tsx`](../renderer/src/components/OnyxSettingsModal.tsx) into tab-level components plus shared settings state/persistence helpers so the modal is no longer a single giant UI surface.
  Current progress: all tab bodies now live in dedicated settings components (`SettingsGeneralTab.tsx`, `SettingsAnimationsTab.tsx`, `SettingsIntegrationsTab.tsx`, `SettingsAboutTab.tsx`, `SettingsLibrariesTab.tsx`, `SettingsAdvancedTab.tsx`, `SettingsLinksTab.tsx`, `SettingsScanningTab.tsx`, and `SettingsSuspendTab.tsx`), and `useOnyxSettingsModalShellState.ts` now owns tab routing, updater/about state, API credential status, and suspend shortcut capture; the remaining work is the app-config/manual-folder/save/reset orchestration still embedded in `OnyxSettingsModal.tsx`.
