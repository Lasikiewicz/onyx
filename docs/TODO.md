...
- [ ] **Finish decomposing `App.tsx`** - Move the remaining shell orchestration in [`App.tsx`](../renderer/src/App.tsx) into focused hooks/components, especially background/media policy, importer handoff state, and any remaining modal-control glue that still makes the file hard to reason about.
  Current progress: animated-media policy now lives in [`useAnimatedMediaPolicy.ts`](../renderer/src/hooks/useAnimatedMediaPolicy.ts) and importer lifecycle/handoff now lives in [`useImporterWorkbench.ts`](../renderer/src/hooks/useImporterWorkbench.ts), so the remaining work is the rest of the shell-level modal/control orchestration.
