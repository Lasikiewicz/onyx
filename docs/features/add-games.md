...
3. The importer scans configured sources or accepts pre-scanned games, showing a lightweight branded scan screen with the compact Onyx cube spinner while it builds a staged `queue` of [`StagedGame`](../../renderer/src/types/importer.ts) records.
...
11. Startup and background scan notifications apply the same persisted ignored-game identities before showing the New Games Found dialog.
