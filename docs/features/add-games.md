...
14. Import only commits staged games with `status === 'ready'` (matching the footer's ready-count label) through the main-process import pipeline into the library — staged games still marked "Attention Needed" are left in the queue rather than silently imported incomplete — after which the importer closes or refreshes surrounding UI state.
