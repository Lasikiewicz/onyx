...
- The Games View editor slice in [`RightClickMenuGamesViewSection.tsx`](../../renderer/src/components/rightClickMenu/RightClickMenuGamesViewSection.tsx), which owns the grid/list/logo/card-poster left-column controls for categories, size, padding, list display options, Card / Poster poster-only mode, and background blur/brightness so the parent menu no longer keeps that games-view editor inline.
