...
- The controller navigation bridge in [`useControllerNavigation.ts`](../../renderer/src/hooks/useControllerNavigation.ts), which owns browser Gamepad API polling, repeat throttling, DualSense-style focus routing between visible games, the details action row, top controls, and shell context menus, plus connection/input status and debug reporting for local controller detection issues.
...
28. [`useControllerNavigation.ts`](../../renderer/src/hooks/useControllerNavigation.ts) routes standard and common non-standard gamepad input, including analog button values and hat axes, through the same active-game, context-menu, top-bar, and details-action callbacks used by mouse input, so controller support does not duplicate shell selection state.
...
- [useControllerNavigation.ts](../../renderer/src/hooks/useControllerNavigation.ts) - browser Gamepad API polling, controller focus routing, connection/input toasts, and debug state for grid/list/logo shell control.
