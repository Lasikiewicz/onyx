...
- `gridSmartFill`/`cardSmartFill` drive the shared [`smartFillColumns.ts`](../../renderer/src/utils/smartFillColumns.ts) algorithm (used by both [`LibraryGrid.tsx`](../../renderer/src/components/LibraryGrid.tsx) and [`LibraryCardView.tsx`](../../renderer/src/components/LibraryCardView.tsx)), which only ever shrinks tiles to fit — it takes a `minColumns` floor derived from the configured `gridSize`/`logoSize`/`cardColumns` so it never enlarges tiles past their configured size for small libraries. Both components also recompute on `window resize` and DPI (`matchMedia('(resolution: ...)')`) changes in addition to `ResizeObserver` (guarded with `typeof window.matchMedia === 'function'` since it isn't available in every environment, e.g. jsdom tests), since moving the window to a display with a different scale factor can change the effective layout without changing the observed container's CSS-pixel box size. `gridSmartFill` defaults to `true` (both in `createDefaultPreferences()` and the 1080p baseline in [`UserPreferencesService.ts`](../../main/UserPreferencesService.ts)); `cardSmartFill` still defaults to `false`.
- `gridMaximizeSpace` (Grid/Logo only, shown in the menu only while `gridSmartFill` is on, defaults to `true`) overrides `smartFillColumns.ts`'s "never grow past configured size" floor and instead calls `computeMaximizeSpaceLayout()` in the same file: it also resizes the Game Details panel (via the `panelWidth`/`onPanelWidthChange` props threaded into [`LibraryGrid.tsx`](../../renderer/src/components/LibraryGrid.tsx) from [`App.tsx`](../../renderer/src/App.tsx)/[`AppShellLibraryView.tsx`](../../renderer/src/components/appShell/AppShellLibraryView.tsx)) so the games view always gets the largest tiles that exactly fill the available height, keeping the panel at least `max(400px, 25%)` and at most `75%` of the total row width (mirroring the manual drag-handle bounds in [`GameDetailsPanel.tsx`](../../renderer/src/components/GameDetailsPanel.tsx)). While `gridMaximizeSpace` is active, [`GameDetailsPanel.tsx`](../../renderer/src/components/GameDetailsPanel.tsx) hides its manual resize divider (`disablePanelResize`, threaded through [`useGameDetailsPanelControls.ts`](../../renderer/src/hooks/useGameDetailsPanelControls.ts)) since Maximize Space owns the panel width.
