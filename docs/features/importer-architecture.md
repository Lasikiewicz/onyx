...
2. Renderer opens [`ImportWorkbenchV2.tsx`](../../renderer/src/components/importer/ImportWorkbenchV2.tsx) and either requests fresh scans or receives pre-scanned game candidates.
...
6. On import, renderer maps staged records into final library-save requests, and main-process game persistence stores them as library [`Game`](../../renderer/src/types/game.ts) records.
7. Post-import follow-up may reload the library, trigger tutorial/state refresh behavior, and send the user back into the main shell or later into [`GameManager.tsx`](../../renderer/src/components/GameManager.tsx) for post-import maintenance.
