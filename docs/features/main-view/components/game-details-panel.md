...
- **Adaptive media layout:** Description content is grouped into section rows (heading/body + nearest media). In wider/taller layouts, each media-bearing section becomes one floated 60%-width image on the left with its matching text block wrapped on the right; sections without media stay full-width and left-aligned. In tighter layouts everything remains stacked and media scales down with bounded height.
