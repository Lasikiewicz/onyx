...
- Shared modal runtime behavior now also flows through [`useOnyxSettingsModalShellState.ts`](../../renderer/src/hooks/useOnyxSettingsModalShellState.ts) so updater/about state, API credentials status, tab routing, and suspend shortcut capture no longer sit inline inside the modal shell.
- Main settings tabs:
