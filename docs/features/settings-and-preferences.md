...
- `detailsPanelMinWidthPercent` (default `25`, range `25`–`50`) is the floor for the Game Details panel width while Maximize Space is active in Grid/Logo views. It is stored alongside `gridSmartFill`/`gridMaximizeSpace` in the grid and logo readable-settings sections, so a custom-defaults export/import carries it.
