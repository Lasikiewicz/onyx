import React, { useMemo } from 'react';


interface MissingGamesModalProps {
    missingGames: Array<{
        id: string;
        title: string;
        exePath?: string;
        platform?: string;
        source?: string;
    }>;
    onRemove: (gameIds: string[]) => void;
    onCancel: () => void;
}

export function MissingGamesModal({ missingGames, onRemove, onCancel }: MissingGamesModalProps) {
    const [selectedGames, setSelectedGames] = React.useState<Set<string>>(
        new Set(missingGames.map(g => g.id))
    );

    const sortedMissingGames = useMemo(() => {
        return [...missingGames].sort((a, b) => a.title.localeCompare(b.title));
    }, [missingGames]);

    const toggleGame = (gameId: string) => {
        setSelectedGames(prev => {
            const newSet = new Set(prev);
            if (newSet.has(gameId)) {
                newSet.delete(gameId);
            } else {
                newSet.add(gameId);
            }
            return newSet;
        });
    };

    const toggleAll = () => {
        if (selectedGames.size === missingGames.length) {
            setSelectedGames(new Set());
        } else {
            setSelectedGames(new Set(missingGames.map(g => g.id)));
        }
    };

    const handleRemove = () => {
        onRemove(Array.from(selectedGames));
    };

    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100]">
            <div className="bg-gradient-to-br from-gray-900 to-slate-950 border border-gray-700/50 rounded-2xl shadow-2xl p-6 max-w-2xl w-full max-h-[80vh] flex flex-col animate-in fade-in zoom-in duration-200">
                <div className="flex items-center gap-4 mb-6">
                    <div className="w-12 h-12 rounded-full bg-red-500/10 flex items-center justify-center border border-red-500/20">
                        <svg className="w-6 h-6 text-red-500 group- hover:animate-wobble group-hover:animate-wobble" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                        </svg>
                    </div>
                    <div>
                        <h2 className="text-2xl font-bold text-white">Missing Games Detected</h2>
                        <p className="text-gray-400 text-sm mt-1">
                            The following games were not found. Would you like to remove them from your library?
                        </p>
                    </div>
                </div>

                <div className="flex items-center justify-between mb-4 px-1">
                    <span className="text-sm font-medium text-gray-400">
                        {missingGames.length} {missingGames.length === 1 ? 'game' : 'games'} found
                    </span>
                    <label className="flex items-center gap-2 cursor-pointer group">
                        <input
                            type="checkbox"
                            checked={selectedGames.size === missingGames.length}
                            onChange={toggleAll}
                            className="w-4 h-4 rounded border-gray-600 bg-gray-700 text-blue-600 focus:ring-blue-500 focus:ring-offset-gray-900"
                        />
                        <span className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">
                            Select All
                        </span>
                    </label>
                </div>

                <div className="flex-1 overflow-y-auto mb-6 pr-2 -mr-2 space-y-2">
                    {sortedMissingGames.map(game => (
                        <div
                            key={game.id}
                            className="flex items-start gap-4 p-4 bg-gray-800/40 border border-gray-700/30 rounded-xl hover:bg-gray-800/60 transition-colors group"
                        >
                            <input
                                type="checkbox"
                                checked={selectedGames.has(game.id)}
                                onChange={() => toggleGame(game.id)}
                                className="mt-1.5 w-4 h-4 rounded border-gray-600 bg-gray-700 text-blue-600 focus:ring-blue-500 focus:ring-offset-gray-900 cursor-pointer"
                            />
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start gap-2">
                                    <div className="font-semibold text-white truncate">{game.title}</div>
                                    {(game.platform || game.source) && (
                                        <div className="px-2 py-0.5 rounded textxs font-medium bg-gray-700 text-gray-300 uppercase tracking-wider text-[10px]">
                                            {game.platform || game.source}
                                        </div>
                                    )}
                                </div>
                                {game.exePath && (
                                    <div className="text-xs text-gray-500 mt-1 font-mono break-all group-hover:text-gray-400 transition-colors">
                                        {game.exePath}
                                    </div>
                                )}
                            </div>
                        </div>
                    ))}
                </div>

                <div className="flex justify-end gap-3 pt-4 border-t border-gray-800">
                    <button
                        onClick={onCancel}
                        className="px-5 py-2.5 bg-gray-800 hover:bg-gray-700 text-white font-medium rounded-lg transition-colors border border-gray-700"
                    >
                        Keep All
                    </button>
                    <button
                        onClick={handleRemove}
                        disabled={selectedGames.size === 0}
                        className="px-5 py-2.5 bg-gradient-to-r from-red-600 to-red-500 hover:from-red-500 hover:to-red-400 text-white font-medium rounded-lg transition-all shadow-lg shadow-red-500/20 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none"
                    >
                        Remove Selected ({selectedGames.size})
                    </button>
                </div>
            </div>
        </div>
    );
}
