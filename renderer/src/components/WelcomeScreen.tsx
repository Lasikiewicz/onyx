...
export const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onScanGames, onAddFolder, onOpenSettings }) => {
    ...
    useEffect(() => {
        ...
        // Listen for modal close events (which might indicate settings were saved)
                    ...
                    {apisConfigured === false ? (
                        <div className="col-span-2">
                            <button
                                ...
                                className="w-full group relative flex flex-col items-center p-8 bg-red-600/10 hover:bg-red-600/20 border border-red-500/30 hover:border-red-500/50 rounded-2xl transition-all duration-300 transform hover:-translate-y-1 hover:shadow-2xl hover:shadow-red-500/10"
                            >
